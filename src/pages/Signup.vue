  ...
  <main class="min-h-screen flex flex-col bg-surface-subtle text-text">
    ...
    <div class="flex-1 flex items-center justify-center page-shell py-16">
      <div class="card w-full max-w-sm p-6 space-y-6">
        <div>
          <h1 class="text-display-md text-text font-semibold">Create account</h1>
          <p class="text-body-sm text-text-dim">We'll store your account in localStorage.</p>
        </div>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <div class="space-y-2">
            ...
            <input id="password" v-model="password" class="w-full rounded-card border border-surface-border bg-surface px-3 py-2 text-body-md text-text focus-visible:ring-brand-500" type="password" placeholder="Create a password" />
        ...
        <div class="text-center text-body-sm text-text-dim">
          ...
          <RouterLink class="text-brand-600 hover:text-brand-700 font-medium focus-visible:ring-2 focus-visible:ring-brand-500 rounded-sm" to="/auth/login">
            Log in
          </RouterLink>
        </div>

        <div class="text-center text-body-xs text-text-dim text-[12px] leading-[16px]">
          After signup you can still test with: test@ticketapp.test / password123
        </div>
      </div>
    </div>
...
const topLevelMessage = computed(() => {
  if (serverErr.value && isValidationError(serverErr.value)) {
...
async function handleSubmit() {
  ...
  const result = await auth.doSignup({ email: email.value.trim(), password: password.value });
  if (!result.ok) {
